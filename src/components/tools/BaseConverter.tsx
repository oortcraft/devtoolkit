...
import { convertBase, BASE_OPTIONS, type BaseType } from '../../lib/base-converter-utils';
import CodeEditor from './CodeEditor';
import CopyButton from './CopyButton';
import ToolErrorBoundary from './ToolErrorBoundary';

function BaseConverterInner() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      const { result, error: err } = convertBase(input, selectedBase);
      if (err) {
        setError(err);
        setOutput('');
      } else {
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2">
          <span className="text-[13px] font-medium text-[var(--color-muted-foreground)]">From:</span>
          <div className="flex flex-wrap gap-1 rounded-md bg-[var(--color-secondary)] p-1">
            {BASE_OPTIONS.map((opt) => (
              <button
                key={opt.value}
                type="button"
                aria-pressed={selectedBase === opt.value}
                aria-label={opt.label + ' base'}
                onClick={() => setSelectedBase(opt.value)}
                className={[
      ...
      <div className="flex flex-col gap-4 md:flex-row">
        ...
        <div className="min-w-0 flex-1">
          ...
          <CodeEditor value={output} language="text" readOnly placeholder="Converted values will appear here" />
        </div>
      </div>
    </div>
  );
